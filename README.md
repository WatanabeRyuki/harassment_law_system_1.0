...
本プロジェクトは、ハラスメント検知ではなく「証拠保全と構造分析」に重点を置いた独自アーキテクチャを採用し、現在は音声入力から分析前処理までの基盤が完成し、最大の独自ロジックである会話構造性分析（S）の実装を終え、法律を付与することでエンジン全体は完成した段階にある。今後はAPIかを行いUIを作成することでWebアプリとして実装し、より評価機能の完成を目指す。
